...
# metadata regressior
partial_fit_classifiers = joblib.load('./bigmetadata/partial_fit_classifiers.joblib')
...
for testdataset in testdatasetnames:
    ...
    # the classify way 
    bigsgd_classify_result = []
    for round in range(splitcount):

        meta_query = QueryMetaData_classify(X, y, partial_fit_classifiers['SGD'], copy.deepcopy(label_index_round[round]), copy.deepcopy(unlabel_index_round[round]), copy.deepcopy(model_output_round[round]))
        # Get the data split of one fold experiment
        train_idx, test_idx, label_ind, unlab_ind = alibox.get_split(round)
        # Get intermediate results saver for one fold experiment
        saver = alibox.get_stateio(round)
        ...
        # Reset the progress in stopping criterion object
        stopping_criterion.reset()
        bigsgd_classify_result.append(copy.deepcopy(saver))

    # the classify way 
    bigPerceptron_classify_result = []
    for round in range(splitcount):

        meta_query = QueryMetaData_classify(X, y, partial_fit_classifiers['Perceptron'], copy.deepcopy(label_index_round[round]), copy.deepcopy(unlabel_index_round[round]), copy.deepcopy(model_output_round[round]))
        # Get the data split of one fold experiment
        train_idx, test_idx, label_ind, unlab_ind = alibox.get_split(round)
        # Get intermediate results saver for one fold experiment
        saver = alibox.get_stateio(round)
        ...
        # Reset the progress in stopping criterion object
        stopping_criterion.reset()
        bigPerceptron_classify_result.append(copy.deepcopy(saver))

    # the classify way 
    bigPassive_Aggressive_classify_result = []
    for round in range(splitcount):

        meta_query = QueryMetaData_classify(X, y, partial_fit_classifiers['Passive-Aggressive'], copy.deepcopy(label_index_round[round]), copy.deepcopy(unlabel_index_round[round]), copy.deepcopy(model_output_round[round]))
        # Get the data split of one fold experiment
        train_idx, test_idx, label_ind, unlab_ind = alibox.get_split(round)
        # Get intermediate results saver for one fold experiment
        saver = alibox.get_stateio(round)
        ...
        # Reset the progress in stopping criterion object
        stopping_criterion.reset()
        bigPassive_Aggressive_classify_result.append(copy.deepcopy(saver))
    ...
    analyser.add_method(method_name='bigsgd_classify', method_results=bigsgd_classify_result)
    analyser.add_method(method_name='bigPerceptron_classify', method_results=bigPerceptron_classify_result)
    analyser.add_method(method_name='bigPassive_Aggressive_classify', method_results=bigPassive_Aggressive_classify_result)
