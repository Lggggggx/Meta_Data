...
class DataSet():
    ...
    def get_distance(self, method='Euclidean'):
        ...
        return self.distance, min_distance, max_distance
    ...
    def get_node_potential(self):
        ...
        References
        ----------
        [1] Ebert S, Fritz M, Schiele B. RALF: A reinforced active learning formulation for object class recognition[C]// 
        ...
        for i in range(self.n_samples):
            for j in range(self.n_samples):
                if i != j:
                    node[i] += np.exp(-(25*np.square(self.distance[i][j])))
    ...
    def get_graph_density(self, k=10):
        ...
        Returns
        -------
        gra_matrix: array-like
            Graph density (Gra) is novel sampling criteria that
            uses a graph structure to identify highly connected nodes.
            shape like [n_samples]
        p_matrix: array-like
            Pij = 1 if d(xi, xj) is one of the k smallest distances of xi 
            with Manhattan distance d and k (default=10) for the number of nearest neighbors.
            shape like [n_samples, n_samples]
        References
        ----------
        [1] Ebert S, Fritz M, Schiele B. RALF: A reinforced active learning formulation for object class recognition[C]// 
            Computer Vision and Pattern Recognition (CVPR), 2012 IEEE Conference on. IEEE, 2012.
        ...
        return gra_matrix, p_matrix
    
    def get_static_meta_features(self):
        '''Get the static meta feature of dataset X.
        Such as simple meta-features, statistical meta-feature, PCA meta-features
        
        Returns:
        --------
        static_meta_feature: list
            shape like [19]
        '''
        # simple meta-features
        simple_mf = np.hstack((self.n_samples, np.log(self.n_samples), self.n_features, np.log(self.n_features), \
            self.n_features/self.n_samples, np.log(self.n_features/self.n_samples), self.n_samples/self.n_features, np.log(self.n_samples/self.n_features)))

        # Statistical meta-features
        skewness_list = skew(self.X)
        kurtosis_list = kurtosis(self.X)
        statistical_mf = np.hstack((np.min(skewness_list), np.max(skewness_list), np.mean(skewness_list), np.std(skewness_list), \
            np.min(kurtosis_list), np.max(kurtosis_list), np.mean(kurtosis_list), np.std(kurtosis_list)))
        
        # PCA meta-features
        pca = PCA(n_components=0.95)
        pca.fit(self.X)
        pca_95perce = np.sum(pca.explained_variance_ratio_)
        pca = PCA(n_components=1)
        pca.fit(self.X)
        new_X = pca.transform(self.X)
        pca_skewness_first_pc = skew(new_X)
        pca_kurtosis_first_pc = kurtosis(new_X)
        pca_mf = np.hstack((pca_95perce, pca_skewness_first_pc, pca_kurtosis_first_pc))
        return np.hstack((simple_mf, statistical_mf, pca_mf))
...
def meta_data(X, y, distance, min_distance, max_distance, node_potential, gra_matrix, p_matrix, cluster_center_index, label_indexs, unlabel_indexs, modelOutput, query_index):
    ...
    # node potential
    query_node_potential = node_potential[query_index]
    for j in label_indexs[5]:
        query_node_potential -= node_potential[j] * np.exp(-(16*np.square(distance[i][j])))

    # kernel_farthest_first
    # the minimum distance between the X[query_index} and the labeled samples
    query_index_kff = -1
    for j in label_indexs[5]:
        if distance[query_index][j] < query_index_kff:
            query_index_kff = distance[query_index][j]
    query_kff = (query_index_kff - min_distance) / (max_distance - min_distance)

    # graph density
    query_graph_den = gra_matrix[query_index]
    for j in label_indexs[5]:
        query_graph_den -= p_matrix[query_index][j] * gra_matrix[j]
    '''
    # RALF: sample criteria 
    query_node_potential = node_potential[query_index]
    query_index_kff = -1
...
def newmeta_data(X, y, distance, min_distance, max_distance, node_potential, gra_matrix, p_matrix, cluster_center_index, label_indexs, unlabel_indexs, modelOutput, query_index):
    """Calculate the meta data according to the current model,dataset and five rounds before information.
    ...
    metadata = np.hstack((query_node_potential, query_kff, query_graph_den, metadata))
    metadata = np.array([metadata])
    return metadata
...
def new_cal_meta_data_sequence(X, y, distance, min_distance, max_distance, node_potential, gra_matrix, p_matrix, cluster_center_index, modelnames, test, label_ind, unlabel_ind, split_count_th, num_xjselect, diff_five_round):
    """calculate the designed mate data. 
    Parameters
    ----------
    X: 2D array, optional (default=None) [n_samples, n_features]
        ...
        for k in range(num_models):
            ...
            for _ in range(diff_five_round):
                ...
                for j in range(num_xjselect):
                    ...
                    # calulate the designed meta_data Z
                    j_meta_data = newmeta_data(X, y, distance, min_distance, max_distance, node_potential, gra_matrix, p_matrix, cluster_center_index, j_labelindex, j_unlabelindex, jmodelOutput, j_sampelindex)

                    # print('j_meta_data')
                    # print(np.shape(j_meta_data))
                    # print(j_meta_data)
                    # calulate the performace improvement
                    j_acc = accuracy_score(y[test], j_prediction[test])
                    j_roc = roc_auc_score(y[test], j_output[test])
...
def new_cal_meta_data_Z(X, y, distance, min_distance, max_distance, node_potential, gra_matrix, p_matrix, cluster_center_index, model, label_inds, unlabel_inds, modelOutput):
    """calculate the designed mate data. 
    Parameters
    ----------
    X: 2D array, optional (default=None) [n_samples, n_features]
...
if __name__ == "__main__":
    ...
    static_mf = dataset.get_static_meta_features()
    print(np.shape(static_mf))
    print(static_mf)
    new= np.repeat([static_mf], 10, axis=0)
    print(np.shape(new))
    print(new[0:2,:])
