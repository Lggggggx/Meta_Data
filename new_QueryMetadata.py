...
from new_meta_data import DataSet, newmeta_data
...
class QueryMetaData():
    ...
    def __init__(self, X, y, metaregressor, label_inds_5=None, unlabel_inds_5=None, modelOutput_5=None):
        ...
        self.distance, self.min_distance, self.max_distance = dt.get_distance()
        self.node_potential = dt.get_node_potential()
        self.gra_matrix, self.p_matrix = dt.get_graph_density()
        self.static_mf = dt.get_static_meta_features()
    ...
    def cal_mate_data_Z(self, label_inds, unlabel_inds, modelOutput, model):
        ...
        -------
        metadata: 2D
            The meta data about the current model and dt.[num_unlabel, 396(features)]
        """
        ...
        for j_sampelindex in unlabel_inds[4]:
            ...
            j_meta_data = newmeta_data(self.X, self.y, self.distance, self.min_distance, \
                 self.max_distance, self.node_potential, self.gra_matrix, self.p_matrix, self.cluster_center_index, \
                      j_labelindex, j_unlabelindex, jmodelOutput, j_sampelindex)
            j_meta_data = np.hstack(([self.static_mf], j_meta_data))
            if metadata is None:
